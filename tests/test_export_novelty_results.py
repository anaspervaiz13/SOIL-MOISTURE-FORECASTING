...
class ArgsStub:
    ...
    lstm_experiment = "lstm__cpu40"
    prophet_experiment = "prophet__safe"
    arima_experiment = "arima__safe180"
    stacking_experiment = "stack_linear__aligned"
...
class TestExportNoveltyResults(unittest.TestCase):
    ...
    def test_build_benchmark_table_uses_best_knn_test_row(self):
        ...
        stacking = root / "stacking" / "stack_linear__aligned"
        for folder in [xgb, knn, lstm, prophet, arima, stacking]:
            folder.mkdir(parents=True, exist_ok=True)
        ...
        write_summary(lstm, rmse=0.0587, mae=0.0487, r2=0.38)
        ...
        (stacking / "run_metadata.json").write_text(
            json.dumps({"selected_knn_neighbor_count": 13}),
            encoding="utf-8",
        )
