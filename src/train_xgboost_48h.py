...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train XGBoost ablation models for 48-hour soil moisture forecasting.")
    parser.add_argument(
        "--feature-group",
...
def main() -> Path:
    ...
    summary = summarize_run_metrics(run_metrics.loc[run_metrics["split"] == "test", ["seed", "rmse", "mae", "r2"]].rename(columns={"seed": "run_id"}))

    output_dir = OUTPUTS_TRAINING_DIR / "xgboost" / resolve_output_dir_name(args.feature_group, args.output_tag)
    metadata = {
        "model": "xgboost",
