...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Transformer for 48-hour soil moisture forecasting.")
...
def build_model(input_shape, heads: int, ff_dim: int):
    import tensorflow as tf

    inputs = tf.keras.layers.Input(shape=input_shape)
    x = tf.keras.layers.LayerNormalization()(inputs)
    attn_output = tf.keras.layers.MultiHeadAttention(num_heads=heads, key_dim=input_shape[-1])(x, x)
...
def main():
    ...
    for run_idx, seed in enumerate(seeds, start=1):
        set_seed(seed)
        model = build_model((X_train.shape[1], X_train.shape[2]), args.heads, args.ff_dim)
        callbacks = [
    ...
    output_dir = OUTPUTS_TRAINING / "transformer"
    metadata = {"model": "transformer", "lookback": args.lookback, "sequence_features": SEQUENCE_FEATURES, "seeds": seeds}
