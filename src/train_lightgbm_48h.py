...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train LightGBM benchmark models for 48-hour soil moisture forecasting.")
    parser.add_argument(
        "--feature-group",
...
def train_one_run(args: argparse.Namespace, feature_columns: list[str], seed: int):
    try:
        ...
        raise SystemExit("lightgbm is not installed. Install it before running this script.") from exc
...
def main() -> Path:
    ...
    output_dir = OUTPUTS_TRAINING_DIR / "lightgbm" / resolve_output_dir_name(args.feature_group, args.output_tag)
    metadata = {
        "model": "lightgbm",
