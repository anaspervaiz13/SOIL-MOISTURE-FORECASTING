...
from src.train_lstm_48h import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train GRU for 48-hour soil moisture forecasting.")
    ...
    parser.add_argument("--gru-units", default="64,32")
    parser.add_argument("--threads", type=int, default=1)
    parser.add_argument("--output-tag", default=None)
    return parser.parse_args()


def build_model(input_shape, gru_units: list[int], dropout: float, learning_rate: float):
    import tensorflow as tf

    layers = [tf.keras.layers.Input(shape=input_shape)]
    for index, units in enumerate(gru_units):
        return_sequences = index < len(gru_units) - 1
        layers.append(tf.keras.layers.GRU(units, return_sequences=return_sequences))
...
def main() -> Path:
    ...
    gru_units = parse_lstm_units(args.gru_units)
    ...
    output_dir = OUTPUTS_TRAINING_DIR / "gru" / resolve_output_dir_name("gru", args.output_tag)
    metadata = {
        "model": "gru",
